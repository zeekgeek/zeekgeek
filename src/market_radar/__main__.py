"""Command-line entry point for the crypto market radar."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Crypto market radar: cryptos across the board plus stock bellwethers"
    )
    parser.add_argument("--host", default="127.0.0.1", help="Dashboard host")
    parser.add_argument("--port", type=int, default=8810, help="Dashboard port")
    parser.add_argument("--demo", action="store_true", help="Simulate a market session instead of polling live feeds")
    parser.add_argument("--poll-interval", type=float, default=60.0, help="Seconds between live feed polls")
    parser.add_argument(
        "--no-auto-demo-fallback",
        action="store_true",
        help="Exit if the live feeds fail instead of switching to demo mode",
    )
...
async def run(args: argparse.Namespace) -> None:
    state = MarketState(data_source="demo" if args.demo else "live")
    ...
    if args.demo:
        print("Running in demo mode: calm drift, risk-off shock, then relief rally.")
    else:
        print("Polling CoinGecko, Stooq, and Fear & Greed with automatic demo fallback if unavailable.")
...
async def _run_feed(
    *,
    ...
    try:
        ...
        await state.set_data_source("demo")
