...
A separate, self-contained radar for **WiFi** devices. It tracks nearby APs by
BSSID, classifies each AP as **stationary** or **moving** from RSSI history,
raises a proximity alarm, and (in monitor mode) captures nearby **client**
activity plus a local AI-style analysis summary.
...
For a live AP scan (Linux, needs the `iw` tool and usually root):

```bash
sudo python3 -m wifi_radar
```

To enable monitor mode (client/AP frame capture + AI analysis):

```bash
sudo python3 -m wifi_radar --monitor-mode
```

If live scanning is unavailable (no `iw`, no wireless interface, missing
permissions, or no `nl80211` support) the app auto-switches to demo mode and
...
## Monitor mode and AI analysis

- `--monitor-mode` attempts to create a virtual monitor interface (`<base>mon`)
  and captures management/data frames with `tcpdump`.
- If monitor interface creation fails, the radar falls back to converting the
  base interface into monitor mode for the session, then restores managed mode
  on exit.
- Monitor captures are used to infer nearby client activity:
  - probing clients (`Probe Request`)
  - clients associated with observed AP BSSIDs
  - recent frame behavior
- The dashboard and `/api/devices` include an `ai_analysis` block generated by
  a local heuristic model (`local-heuristic-v1`) with risk level, highlights,
  and recommendations.

## Command options

```text
python3 -m wifi_radar --host 127.0.0.1 --port 8770 --alarm-range 5 --scan-interval 3 --monitor-mode
```
...
- `--monitor-mode`: enable monitor mode and capture client/AP frames
- `--monitor-interface`: preferred monitor interface name (default `<base>mon`)
- `--monitor-capture-seconds`: monitor capture duration per loop
...
- `GET /api/devices`: APs, clients, monitor status, alarms, and `ai_analysis`
