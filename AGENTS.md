...
- `trace_radar` — PingPlotter-style hop health radar (timeline, WHOIS/DNS/ports/ping, 3D globe)
...
- Trace radar: `python3 -m trace_radar --demo` (PingPlotter-style hop timeline + WHOIS/DNS/
 port scan/ping). Live mode shells out to `traceroute`/`tracepath` (or Python UDP fallback)
 and enriches hops via ip-api.com + rdap.org. Default dashboard port is `8800`.
