...
- `trace_radar` — visual 3D traceroute radar (packet-loss %, WHOIS/RDAP, speed test)
...
- Trace radar: `python3 -m trace_radar --demo` (scripted multi-hop routes with packet-loss %
 and WHOIS). Live mode shells out to `traceroute`/`tracepath` and enriches hops via
 ip-api.com + rdap.org. Default dashboard port is `8800`.
